  ...
  void SpotMicroWalkState::updatePhaseData() {
    ...
    for (int i = 0; i < smnc_.num_phases; i++) {
      ...
      if (phase_time < phase_sum) {
        phase_index_ = i;
        subphase_ticks_ = phase_time - phase_sum + smnc_.phase_ticks[i]; //在當前相位的幾% 
        break;
      }
    }
  ...
  smk::LegsFootPos SpotMicroWalkState::stepGait(
    ...
    for (int i = 0; i < 4; i++) {
      if (i == 0) { // right back
        contact_mode = contact_feet_states_.right_back_in_swing; //是否踩地 True 空中 false 地上 
        foot_pos = body_state.leg_feet_pos.right_back;//目前位置
        default_stance_foot_pos = default_stance_feet_pos.right_back;//目標位置
      ...
      if (contact_mode == false) { // Stance controller
        ...
        swing_proportion = (float)subphase_ticks_ / (float)smnc.swing_ticks; //算相位百分比 
  ...
  smk::Point SpotMicroWalkState::stanceController( //往後幹 
    ...
    Vector3f delta_pos(-cmd.getXSpeedCmd() * dt, // x加負號 因為要往前 實際上是往後跑
                      (1.0f/h_tau)*(0.0f - foot_pos.y) * dt, 
                      -cmd.getYSpeedCmd() * dt);// z負號 同理 

    // Create rotation matrix for yaw rate
    Matrix3f rot_delta;
    rot_delta = AngleAxisf(cmd.getYawRateCmd() * dt, Vector3f::UnitY()); //Vector3f::UnitY() 代表旋轉軸是Y軸
    /* AngleAxisf 會生成3x3旋轉矩陣  
        cosθ 0 sinθ
        0    1   0
      -sinθ    cosθ*/

    // Move foot by rotation and linear translation deltas
    new_foot_pos_vec = (rot_delta * foot_pos_vec) + delta_pos; //先用當前座標成以旋轉矩陣 再加上移動產生的直線座標改變量
  ...
  smk::Point SpotMicroWalkState::swingLegController(
    ...
    if (swing_proportion < 0.5f) {
      ...
      swing_height = (1.0f - (swing_proportion - 0.5f) / 0.5f) * smnc.z_clearance;
    }
    ...
    Vector3f delta_pos(alpha * stance_ticks * dt * cmd.getXSpeedCmd(), //stance_ticks * dt單次採地上的時間
                      0.0f, 
...
//  算速度：拿「目前腳掌與終點站的殘餘距離」除以「剩下的時間」，算出這一瞬間該跨出的步長
